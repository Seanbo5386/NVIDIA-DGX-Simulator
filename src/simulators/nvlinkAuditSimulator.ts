...
export class NvlinkAuditSimulator extends BaseSimulator {
  getMetadata(): SimulatorMetadata {
    return {
      ...
      commands: [
        {
          name: 'nvlink-audit',
          description: 'Run NVLink fabric diagnostics and generate audit report',
   ...
   */
  private handleNvlinkAudit(parsed: ParsedCommand, context: CommandContext): CommandResult {
    ...
    gpusToAudit.forEach(gpu => {
      ...
      if (verbose || checkAll) {
        ...
        for (let link = 0; link < 12; link++) {
          ...
          const statusIcon = linkState.active ? '\x1b[32m✓\x1b[0m' : '\x1b[31m✗\x1b[0m';
          const peerInfo = linkState.peer !== null ? `GPU ${linkState.peer}` : 'NVSwitch';
    ...
    if (checkAll) {
      ...
      for (let i = 0; i < Math.min(gpusToAudit.length, 4); i++) {
        for (let j = i + 1; j < Math.min(gpusToAudit.length, 4); j++) {
          ...
          const expected = 200;
          const statusIcon = bw >= expected * 0.9 ? '\x1b[32mPASS\x1b[0m' : '\x1b[33mWARN\x1b[0m';
    ...
    if (hasErrors) {
      ...
      output += 'Action Required: Investigate NVLink errors and consider GPU reset or reseat\n';
    } else if (hasWarnings) {
      output += '\x1b[33mStatus: WARNINGS\x1b[0m\n';
      output += 'Recommendation: Monitor for degradation, consider preventive maintenance\n';
    } else {
  ...
  private getGpuNvlinkStatus(gpu: any): {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const linkErrors = hasNvlinkFault && i >= 8 ? Math.floor(Math.random() * 100) : 0;
