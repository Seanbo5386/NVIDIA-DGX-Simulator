...
import { useLearningStore, type GauntletAttempt } from "../learningStore";
...
describe("Learning Store", () => {
  ...
  describe("Command Tracking", () => {
    ...
    it("should update mastery level based on success rate", () => {
      ...
      const state = useLearningStore.getState();
      expect(state.commandProficiency["nvidia-smi"].masteryLevel).toBe(
        "expert",
      );
    });
    ...
    it("should track streak count", () => {
      ...
      expect(stateAfterFail.commandProficiency["nvidia-smi"].streakCount).toBe(
        0,
      );
    });
  ...
  describe("Question Tracking", () => {
    ...
    it("should update lastStudied timestamp", () => {
      ...
      const state = useLearningStore.getState();
      expect(state.domainProgress.domain1.lastStudied).toBeGreaterThanOrEqual(
        before,
      );
    });
  });
