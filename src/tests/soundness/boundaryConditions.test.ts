...
describe('Boundary Conditions', () => {
  ...
  describe('GPU Index Boundaries', () => {
    ...
    it('GPU index 8 is invalid (out of bounds)', () => {
      ...
      expect(result.exitCode !== 0 || result.output.toLowerCase().includes('invalid')).toBe(true);
    });
  ...
  describe('nvidia-smi Query Format Boundaries', () => {
    ...
    it('--query-gpu with multiple valid fields works', () => {
      const result = nvidiaSmi.execute(
        parse('nvidia-smi --query-gpu=name,temperature.gpu,memory.used --format=csv'),
        context
      );
      expect(result.exitCode).toBe(0);
    });
  ...
  describe('Node ID Boundaries', () => {
    it('first node (dgx-00) exists', () => {
      const node = useSimulationStore.getState().cluster.nodes.find(n => n.id === 'dgx-00');
    ...
    it('non-existent node ID returns undefined', () => {
      const node = useSimulationStore.getState().cluster.nodes.find(n => n.id === 'dgx-999');
  ...
  describe('Command Parser Edge Cases', () => {
    ...
    it('command with quoted arguments is handled', () => {
      const parsed = parse('scontrol update nodename=dgx-00 state=drain reason="test maintenance"');
