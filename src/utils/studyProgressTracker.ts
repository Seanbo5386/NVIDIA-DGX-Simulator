...
function getInitialProgress(): StudyProgress {
  const domainTrends: Record<DomainId, DomainTrend> = {} as Record<DomainId, DomainTrend>;

  (['domain1', 'domain2', 'domain3', 'domain4', 'domain5'] as DomainId[]).forEach(domain => {
    domainTrends[domain] = {
      domain,
 ...
 */
function updateDomainTrends(progress: StudyProgress, breakdown: ExamBreakdown): void {
...
function updateStreak(progress: StudyProgress): void {
  ...
  const weekSessions = progress.sessions.filter(s => s.timestamp >= weekStart.getTime());
...
function updateAggregateStats(progress: StudyProgress): void {
  const examSessions = progress.sessions.filter(s => s.type === 'exam' || s.type === 'quiz');
  ...
  progress.totalStudyTime = progress.sessions.reduce((acc, s) => acc + (s.duration || 0), 0);
}
...
export function getStudyRecommendations(): string[] {
  ...
  const declining = Object.values(progress.domainTrends)
    .filter(t => t.trend === 'declining' && t.totalAttempts >= 3);
  ...
  const notAttempted = Object.values(progress.domainTrends)
  ...
  } else if (progress.streak.currentStreak === 0 && progress.streak.longestStreak > 0) {
    recommendations.push(
  ...
  if (remaining > 0) {
    ...
    recommendations.push(`You've met your weekly study goal! Consider increasing it for more practice.`);
  }
...
export function getProgressSummary(): {
  ...
  const withAttempts = Object.values(progress.domainTrends).filter(t => t.totalAttempts > 0);
  const sorted = [...withAttempts].sort((a, b) => a.averageScore - b.averageScore);
  ...
  return {
    ...
    strongestDomain: sorted.length > 0 ? sorted[sorted.length - 1].domainName : null,
  };
}
